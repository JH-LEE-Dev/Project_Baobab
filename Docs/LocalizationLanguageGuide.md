...
> ⚠️ 이번 작업에서 `Français`, `Português`, `Español`, `Русский`의
> `ç ê ñ Р у с к и й` 9자가 새로 필요해졌습니다. **(2)를 아직 돌리지 않았으므로,
> 지금 상태로 일본어·중국어로 플레이하면 옵션의 언어 이름이 깨져 보입니다.**
> 다른 작업과 별개로 이건 한 번 돌려주세요.

> 독일어·프랑스어·포르투갈어·스페인어·러시아어로 **플레이할 때의 본문**은 이 과정이 필요 없습니다.
> 갈무리11이 라틴 확장(ä, ç, ã, ñ, œ …)과 키릴 문자를 모두 갖고 있고,
> `Galmuri11_Optimum`은 동적 아틀라스라 런타임에 알아서 채워 넣습니다.
> (2)가 필요한 건 **CJK로 볼 때 섞여 나오는 글자**뿐입니다.

> 독일어·프랑스어·포르투갈어·스페인어·러시아어는 이 과정이 필요 없습니다.
> 갈무리11이 라틴 확장(ä, ç, ã, ñ, œ …)과 키릴 문자를 모두 갖고 있고,
> `Galmuri11_Optimum`은 동적 아틀라스라 런타임에 알아서 채워 넣습니다.
