...
public struct SYSTEM_VAR
{
    ...
    ///
    /// 코드는 하나도 지우지 않고 "들어오는 길"만 막아둔 것이다. 끊어둔 지점은 아래 두 곳이며,
    /// 다시 켜려면 이 값을 true로 바꾸기만 하면 된다(이 상수를 grep하면 전부 찾을 수 있다).
    ///   1. InDungeonObjectManager.OnTreeDead  : 보석 나무가 원석 대신 원목을 떨어뜨린다(원래 동작).
    ///   2. BlastFurnaceManager.IncreaseFurnaceCount : 용광로 특성이 용광로를 열지 못하게 한다.
    ///
    /// 용광로 HUD는 따로 막지 않았다. 해금 수가 0이면 NotifyUIState가 모두 unlocked = false인
    /// 목록을 보내고, 그건 특성을 하나도 찍지 않은 상태와 똑같다.
    ///
    /// <b>원석 재화 HUD는 이 스위치로 되살아나지 않는다.</b> 처음에는 UI_Inventory에서 칸을
    /// 숨기는 방식으로 막아두었는데, 나중에 들어온 작업(009aaaac)이 황금/다이아/프리즘 칸을
    /// 코드와 UI_Inventory 프리팹 양쪽에서 통째로 들어냈다. 계열을 되살릴 때는 그 표시를
    /// 다시 만들어야 한다.
    ///
