...
/// <summary>
/// 메인 카메라의 절반 폭(halfWidth)을 기준으로, 아이소메트릭 2D 시점에 맞는 타원형 사거리를
...
public static class CameraBoundsUtil
{
    ...
    /// 장축(가로)은 카메라 절반 폭(orthographicSize * aspect)의 _majorAxisRatio 배이고,
    /// 단축(세로)은 아이소메트릭 2:1 비율에 따라 장축의 절반이다. _edgePadding만큼 장축을
    /// 줄여 안쪽으로 여유를 둔다. 카메라를 찾을 수 없거나 원근 카메라면 0을 반환한다.
    /// </summary>
