...
public class InDungeonSystem : MonoBehaviour
{
    ...
    /// 구름 걷힘 코루틴(RollbackCameraMoveRoutine의 고정 타이머)을 걸어둬서, Town 초기화가 그 타이머보다
    /// 오래 걸리면 초기화 도중 구름이 걷혀버릴 수 있는 시간 경쟁 구조였다. 다른 전환 경로(Town→Dungeon, Retry 등)와 동일하게
    ...
    private IEnumerator TutorialRideExitCoroutine()
    {
        ...
        // 튜토리얼에서 캐릭터가 차량에서 내리는 순간
        Rumble.Play(EHapticEvent.VehicleDismount);
