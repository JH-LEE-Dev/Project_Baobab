...
public class TilemapForestGenerator : EditorWindow
{
    ...
    public static void GenerateForest()
    {
        ...
        if (tilemapObj == null)
        {
            Debug.LogError("Grid/Tilemap을 찾을 수 없습니다!");
            return;
        }
        ...
        // Undo 기록 시작
        Undo.RecordObject(tilemap, "Generate Forest Tiles");
        ...
        for (int x = fillBounds.xMin; x < fillBounds.xMax; x++)
        {
            for (int y = fillBounds.yMin; y < fillBounds.yMax; y++)
            {
                ...
                if (!tilemap.HasTile(pos))
                {
                    ...
                    if (noiseValue < 0.3f) selectedTile = waterTile;
                    else if (noiseValue > 0.7f) selectedTile = rockTile;
                    else selectedTile = grassTile;
                ...
                if (tilemap.GetTile(pos) == grassTile)
                {
                    // 펄린 노이즈 대신 단순 랜덤을 사용하여 30% 밀도로 배치
                    if (Random.value < 0.3f) 
                    {
                        Vector3 worldPos = tilemap.GetCellCenterWorld(pos);
                        
                        // 정확히 타일 중앙에서 Y축으로 타일 크기의 절반만큼 오프셋 적용
                        worldPos.y += tilemap.cellSize.y * 0.5f;
        ...
        Debug.Log($"자연 환경 생성 완료: {tileCount}개의 타일 배치, {treeCount}그루의 나무 심기 완료 (밀도 약 30%).");
