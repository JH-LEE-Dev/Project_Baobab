...
public class HUD_PopupNav_Main : MonoBehaviour
{
    ...
    private static ENavFocusArea runtimeLastHoveredArea = ENavFocusArea.None;
    private static MapType runtimeLastHoveredMapType = MapType.None;
    private static ForestType runtimeLastHoveredForestType = ForestType.None;
    ...
    private bool isRegionUnlockActive = false;
    private bool isPendingSubRegionUnlock = false;
    ...
    {
        runtimeLastHoveredArea = ENavFocusArea.None;
        runtimeLastHoveredMapType = MapType.None;
        runtimeLastHoveredForestType = ForestType.None;
        runtimeLastSelectedMapType = MapType.None;
        runtimeLastVisitedMapType = MapType.None;
        runtimeLastVisitedForestType = ForestType.None;
    }
    ...
    private void ResetOpenState()
    {
        ...
        // 아래 직접 대입으로 플래그를 지우기 전에, 짝을 잃은 잠금이 있으면 먼저 갚는다.
        // 그냥 지우면 EndUnlockProduction()의 자체 가드(isUnlockingProductionActive)에 걸려
        // 그 +1을 갚을 방법이 영영 사라진다 - 재오픈해도 회수되지 않는다는 뜻이다.
        // (정상 흐름에서는 플래그가 이미 false라 아무 일도 하지 않는다)
        ReleaseStrandedUnlockProductionLock();
    ...
    private void SaveLastHoverState()
    {
        // 1. 게임패드 모드인 경우: 마우스 화면 좌표를 완전히 배제하고 오직 패드 포커스 상태만 반영
        if (null != inputManager && true == inputManager.IsGamepadMode)
        {
            if (ENavFocusArea.SubRegionList == currentFocusArea && null != subRegionGroup)
            {
                ...
                if (null != _subs && 0 <= focusedSubRegionIndex && focusedSubRegionIndex < _subs.Count && null != _subs[focusedSubRegionIndex] && true == _subs[focusedSubRegionIndex].IsUnlocked)
                {
                    runtimeLastHoveredArea = ENavFocusArea.SubRegionList;
                    runtimeLastHoveredMapType = currentSelectedMapType;
                    runtimeLastHoveredForestType = _subs[focusedSubRegionIndex].GetForestType();
            ...
            if (null != regionGroup)
            {
                ...
                if (null != _regions && 0 <= focusedRegionIndex && focusedRegionIndex < _regions.Count && null != _regions[focusedRegionIndex])
                {
                    runtimeLastHoveredMapType = _regions[focusedRegionIndex].GetMapType();
                }
                else
                {
                    runtimeLastHoveredMapType = currentSelectedMapType;
        ...
        if (null != subRegionGroup)
        {
            HUD_PopupNav_SubRegionBtn _hoveredSub = subRegionGroup.GetHoveredSubRegionButton();
            if (null != _hoveredSub && true == _hoveredSub.IsUnlocked)
            {
                runtimeLastHoveredArea = ENavFocusArea.SubRegionList;
                runtimeLastHoveredMapType = currentSelectedMapType;
                runtimeLastHoveredForestType = _hoveredSub.GetForestType();
                runtimeLastVisitedForestType = runtimeLastHoveredForestType;
                return;
            }
        }
        ...
        if (null != regionGroup)
        {
            ...
            if (null != _hoveredRegion && true == _hoveredRegion.IsUnlocked && false == IsDemoRestrictedMapType(_hoveredRegion.GetMapType()))
            {
                runtimeLastHoveredArea = ENavFocusArea.RegionList;
                runtimeLastHoveredMapType = _hoveredRegion.GetMapType();
                runtimeLastHoveredForestType = ForestType.None;
                return;
            }
        }

        if (MapType.None != currentSelectedMapType)
        {
            runtimeLastHoveredArea = ENavFocusArea.RegionList;
            runtimeLastHoveredMapType = currentSelectedMapType;
            runtimeLastHoveredForestType = ForestType.None;
    ...
    private void CloseMainPopup(bool _isInstant = false, bool _playCloseSound = true)
    {
        ...
        if (null != subRegionGroup)
        {
            subRegionGroup.StopAllAnimations();
    ...
    private void ProcessNextUnlock()
    {
        ...
        if (true == _hasCurrentMapSubRegionUnlock)
        {
            ...
            {
                // 소지역 버튼들이 아직 등장 중이므로, 등장 연출 완료(OnSubRegionsShown) 시점까지 대기
                isPendingSubRegionUnlock = true;
                return;
            }

            // 트랜지션이 이미 완료된 상태라면 짧은 딜레이 후 바로 연출 진행
            float _delay = 0.1f;
    ...
    private bool IsSubRegionActuallyUnlocked(MapType _mapType, ForestType _forestType)
    {
        if (null == mapDataProvider) return false;
        MapEnvironmentDatabase _db = mapDataProvider.GetMapEnvironmentDatabase();
        if (null == _db.mapDatas) return false;

        for (int i = 0; i < _db.mapDatas.Count; i++)
        {
            if (_mapType == _db.mapDatas[i].mapType)
            {
                List<ForestEnvironmentInfo> _forests = _db.mapDatas[i].forestDatas;
                if (null == _forests) return false;
                for (int j = 0; j < _forests.Count; j++)
                {
                    if (_forestType == _forests[j].forestType)
                    {
                        return _forests[j].isUnlocked;
                    }
                }
                break;
            }
    ...
    {
        // 잔류된 소지역 세션 기록이 현재 세이브에서 실제로 해금된 상태인지 검증 (잠긴 소지역 기록 무효화)
        if (ENavFocusArea.SubRegionList == runtimeLastHoveredArea)
        {
            if (ForestType.None == runtimeLastHoveredForestType || false == IsSubRegionActuallyUnlocked(runtimeLastHoveredMapType, runtimeLastHoveredForestType))
            {
                runtimeLastHoveredArea = ENavFocusArea.None;
                runtimeLastHoveredMapType = MapType.None;
                runtimeLastHoveredForestType = ForestType.None;
        ...
        else if (ForestType.None != runtimeLastHoveredForestType)
        {
            if (false == IsSubRegionActuallyUnlocked(runtimeLastHoveredMapType, runtimeLastHoveredForestType))
            {
                runtimeLastHoveredArea = ENavFocusArea.None;
                runtimeLastHoveredMapType = MapType.None;
                runtimeLastHoveredForestType = ForestType.None;
        ...
        if (ForestType.None != runtimeLastVisitedForestType)
        {
            if (false == IsSubRegionActuallyUnlocked(runtimeLastVisitedMapType, runtimeLastVisitedForestType))
            {
                runtimeLastVisitedForestType = ForestType.None;
            }
        }
        ...
        // [규칙 3] 1번에서 언급된 Hover 위치가 저장되어 있지 않을 땐
        // 내가 갈 수 있는 최상 대지역에 Hover된다. (게임 처음 시작 or 메인 후 이어하기 상황)
    ...
    private void OnSubRegionsShown()
    {
        ...
        // 대기 중인 소지역 해금 연출이 있다면 지금 시작
        if (true == isPendingSubRegionUnlock)
        {
            isPendingSubRegionUnlock = false;
            float _delay = 0.1f;
            delayedCallTween = DOVirtual.DelayedCall(_delay, onSubRegionUnlockDelayCompleteCallback).SetEase(Ease.Linear);
        ...
        // 해금 연출이 진행 중이거나 대기 중인 경우 포커스를 잡지 않고 대기
        if (true == isUnlockingProductionActive || 0 < regionUnlockList.Count || true == isRegionUnlockActive)
        {
            return;
        }
        ...
        if (null != inputManager && true == inputManager.IsGamepadMode)
        {
            if (ENavFocusArea.SubRegionList == runtimeLastHoveredArea && null != subRegionGroup)
            {
                ForestType _targetForest = runtimeLastHoveredForestType;
                if (ForestType.None != _targetForest)
                {
                    int _subIdx = subRegionGroup.GetSubRegionIndex(_targetForest);
                    if (0 <= _subIdx)
                    {
                        IReadOnlyList<HUD_PopupNav_SubRegionBtn> _subs = subRegionGroup.GetActiveSubRegionButtons();
                        if (null != _subs && _subIdx < _subs.Count && null != _subs[_subIdx] && true == _subs[_subIdx].IsUnlocked)
                        {
                            currentFocusArea = ENavFocusArea.SubRegionList;
                            focusedSubRegionIndex = _subIdx;
            ...
            // 이전 유효한 소지역 호버 기록이 없거나 잠겨있는 경우 소지역 호버 효과 차단 및 대지역 포커스 유지
            if (null != subRegionGroup)
            {
                subRegionGroup.StopAllHoverEffects();
            }
            ...
            if (null != regionGroup)
            {
                int _expectedIdx = regionGroup.GetActiveRegionIndex(currentSelectedMapType);
                if (0 <= _expectedIdx)
                {
                    focusedRegionIndex = _expectedIdx;
                }
                else
                {
                    IReadOnlyList<HUD_PopupNav_RegionBtn> _activeRegions = regionGroup.GetActiveRegionButtons();
                    if (null != _activeRegions && (0 > focusedRegionIndex || focusedRegionIndex >= _activeRegions.Count))
                    {
    ...
    public void HandleSubRegionHovered(ForestType _forestType, Transform _subRegionTransform, ForestEnvironmentInfo _info)
    {
        ...
        runtimeLastHoveredArea = ENavFocusArea.SubRegionList;
        runtimeLastHoveredMapType = currentSelectedMapType;
        runtimeLastHoveredForestType = _forestType;
...
#region Gamepad Navigation
    ...
    public MapType GetHighestPlayableUnlockedRegion(bool _excludePendingUnlocks = false)
    {
        if (null == mapDataProvider) return MapType.None;

        MapEnvironmentDatabase _db = mapDataProvider.GetMapEnvironmentDatabase();
        if (null == _db.mapDatas) return MapType.None;

        MapType _highest = MapType.None;

        for (int i = 0; i < _db.mapDatas.Count; i++)
        {
            MapEnvironmentDataInfo _data = _db.mapDatas[i];
            if (MapType.Town == _data.mapType) continue;
            if (true == IsDemoRestrictedMapType(_data.mapType)) continue;

            if (true == _excludePendingUnlocks)
            {
                bool _isPending = false;
                for (int p = 0; p < regionUnlockList.Count; p++)
                {
                    if (regionUnlockList[p].mapType == _data.mapType)
                    {
                        _isPending = true;
                        break;
                    }
                }
                if (true == _isPending) continue;
            }

            if (true == _data.isUnlocked)
            {
                _highest = _data.mapType;
            }
        }
    ...
    public int GetHighestUnlockedAndPlayableRegionIndex(bool _excludePendingUnlocks = false)
    {
        ...
        {
            HUD_PopupNav_RegionBtn _btn = _activeRegions[i];
            if (null == _btn) continue;

            MapType _map = _btn.GetMapType();
            if (true == IsDemoRestrictedMapType(_map)) continue;
            ...
            if (true == _excludePendingUnlocks)
            {
                ...
                for (int p = 0; p < regionUnlockList.Count; p++)
                {
                    if (regionUnlockList[p].mapType == _map)
                    {
                        _isPending = true;
                        break;
                    }
            ...
            if (true == _btn.IsUnlocked)
            {
                _highestIdx = i;
            }
        }

        return (0 <= _highestIdx) ? _highestIdx : 0;
    }
    ...
    private void SetupInitialGamepadFocus()
    {
        ...
        // [규칙 1 & 2] 마지막 Hover 위치가 소지역이었던 경우:
        if (ENavFocusArea.SubRegionList == runtimeLastHoveredArea && null != subRegionGroup)
        {
            ForestType _targetForest = runtimeLastHoveredForestType;
            if (ForestType.None != _targetForest)
            {
                int _visitedSubIdx = subRegionGroup.GetSubRegionIndex(_targetForest);
                if (0 <= _visitedSubIdx)
                {
                    IReadOnlyList<HUD_PopupNav_SubRegionBtn> _subs = subRegionGroup.GetActiveSubRegionButtons();
        ...
        // [규칙 1 & 2] 마지막 Hover 위치가 대지역이었던 경우:
        if (ENavFocusArea.RegionList == runtimeLastHoveredArea && null != regionGroup)
        {
            MapType _targetMap = runtimeLastHoveredMapType;
            if (MapType.None != _targetMap && false == IsDemoRestrictedMapType(_targetMap))
            {
                int _curIdx = regionGroup.GetActiveRegionIndex(_targetMap);
                IReadOnlyList<HUD_PopupNav_RegionBtn> _regions = regionGroup.GetActiveRegionButtons();
                if (0 <= _curIdx && null != _regions && _curIdx < _regions.Count && null != _regions[_curIdx] && true == _regions[_curIdx].IsUnlocked)
                {
                    currentFocusArea = ENavFocusArea.RegionList;
        ...
        if (null != regionGroup)
        {
            ...
            {
                _targetMap = GetHighestPlayableUnlockedRegion(_excludePendingUnlocks: false);
                currentSelectedMapType = _targetMap;
            }
            ...
            {
                focusedRegionIndex = GetHighestUnlockedAndPlayableRegionIndex(_excludePendingUnlocks: false);
            }
    ...
    private void HandleRegionNavigationInput(Vector2 _input)
    {
            ...
            if (0 <= focusedRegionIndex && focusedRegionIndex < _activeRegions.Count)
            {
                ...
                if (null != _curBtn && true == _curBtn.IsUnlocked)
                {
                    ...
                    if (null != subRegionGroup)
                    {
                        ...
                        IReadOnlyList<HUD_PopupNav_SubRegionBtn> _subs = subRegionGroup.GetActiveSubRegionButtons();
                        if (null != _subs && 0 <= focusedSubRegionIndex && focusedSubRegionIndex < _subs.Count && null != _subs[focusedSubRegionIndex])
                        {
                            runtimeLastHoveredArea = ENavFocusArea.SubRegionList;
                            runtimeLastHoveredMapType = currentSelectedMapType;
                            runtimeLastHoveredForestType = _subs[focusedSubRegionIndex].GetForestType();
        ...
        if (_prevIdx != _nextIdx)
        {
            ...
            if (null != _candidateBtn)
            {
                ...
                runtimeLastHoveredArea = ENavFocusArea.RegionList;
                runtimeLastHoveredMapType = _candidateBtn.GetMapType();
                runtimeLastHoveredForestType = ForestType.None;
            }
        }
    }
    ...
    private void HandleSubRegionNavigationInput(Vector2 _input)
    {
        ...
        runtimeLastHoveredArea = ENavFocusArea.SubRegionList;
        runtimeLastHoveredMapType = currentSelectedMapType;
        runtimeLastHoveredForestType = _candidateBtn.GetForestType();
        runtimeLastVisitedForestType = runtimeLastHoveredForestType;
    }
    ...
    /// <summary>
    /// 해금 연출이 걸어둔 상호작용 키 잠금 중 <b>짝을 잃은 것</b>을 반납합니다.
    /// 파괴(OnDestroy)와 재오픈(ResetOpenState) 두 곳에서 부릅니다.
    ///
    /// StartUnlockProduction()이 올린 +1은 EndUnlockProduction()이 내려야 짝이 맞는데,
    /// 이 컴포넌트가 연출 도중 파괴되면 그 호출이 영영 오지 않습니다. InputReader는
    /// InputManager(BootStrap)가 들고 있어 씬보다 오래 살기 때문에, 그 +1은 메인 메뉴를 거쳐
    /// 다음 세션까지 따라가 상호작용 키를 통째로 죽입니다.
    /// (같은 실패를 막는 코드가 InDungeonSystem/TownSystem의 귀환 코루틴에도 들어 있습니다)
    ///
    /// EndUnlockProduction()을 그대로 부르지 않는 이유가 둘 있습니다.
    ///  1. 그쪽은 OnUnlockProductionEnded 이벤트로 잠금을 되돌리는데, 그 구독자(UIView_MenuPopup)는
    ///     자기 OnDestroy에서 구독을 끊습니다. 두 컴포넌트의 OnDestroy 순서는 보장되지 않으므로
    ///     이벤트에 기대면 반납이 조용히 유실될 수 있습니다.
    ///  2. 그쪽은 호버/패드 포커스 재평가까지 하는데, 파괴 중에는 그 대상들이 이미 없을 수 있습니다.
    ///
    /// 그래서 여기서는 잠금만 직접 갚습니다. 현재 OnUnlockProductionEnded의 구독자는
    /// GameplayUICoordinator 하나이고 그 핸들러가 하는 일도 PauseInteractKey(false) 하나뿐이라
    /// 결과가 같습니다. 구독자가 늘어나면 이 메서드도 함께 손봐야 합니다.
    ///
    /// 평상시에는 아무 일도 하지 않습니다(연출 중 닫기가 UIView_MenuPopup.Hide()의 조기 return과
    /// IsInputBlocked 가드로 막혀 있어, 실제로 플래그가 선 채 이 지점에 오는 경로는 현재 없습니다).
    /// </summary>
    private void ReleaseStrandedUnlockProductionLock()
    {
        if (false == isUnlockingProductionActive)
        {
            return;
    ...
    {
        // inputManager를 null로 비우는 아래 정리보다 반드시 먼저 부른다.
        ReleaseStrandedUnlockProductionLock();
