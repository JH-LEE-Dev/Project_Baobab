Shader "Custom/OutlineStencilWriter-Tree"
{
     Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings vert(Attributes IN)
            {
                ...
                // 쿼드 확장을 먼저 하고 sway 를 둘 다에 같이 적용해야
                // 흔들려도 여백과 원본의 관계가 유지된다.
                ...
                float enableWindSway = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _EnableWindSway);
                float swayPositionAmplitude = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _SwayPositionAmplitude);
                float swayRotationAmplitude = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _SwayRotationAmplitude);
                float swayMainSpeed = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _SwayMainSpeed);
                float swayDetailSpeed = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _SwayDetailSpeed);
                float swayDetailWeight = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _SwayDetailWeight);

                float3 swayedPos = ApplyWindSway(
                    expandedPositionOS, objectPivotWorldPos,
                    enableWindSway, swayPositionAmplitude, swayRotationAmplitude,
                    swayMainSpeed, swayDetailSpeed, swayDetailWeight);
                float3 swayedBasePos = ApplyWindSway(
                    basePositionOS, objectPivotWorldPos,
                    enableWindSway, swayPositionAmplitude, swayRotationAmplitude,
                    swayMainSpeed, swayDetailSpeed, swayDetailWeight);

                OUT.positionHCS = TransformObjectToHClip(swayedPos);
                OUT.worldPos = TransformObjectToWorld(swayedPos);
                OUT.baseWorldPos = TransformObjectToWorld(swayedBasePos).xy;
