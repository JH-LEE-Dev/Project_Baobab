...
`Tools > 빌드 > 배포용 빌드 실행` 을 쓰세요. 경로와 압축(LZ4HC)을 코드가 정합니다.

**유니티의 Build 버튼을 쓰지 마세요.** 그쪽은 압축 방식과 출력 경로를 Build Profile 에서
읽는데, 그 프로필은 `Library` 아래라 **저장소에 올라가지 않습니다.** 사람마다 값이 다르고
새로 클론한 사람은 기본값으로 시작합니다. 실제로 압축이 꺼진 채 itch 빌드가 나가
242MB(정상 151MB)가 된 적이 있습니다. 에셋이 낱개로 풀려 꺼내기도 쉬워집니다.
...
`Tools > 빌드 > 배포용 빌드 실행` 을 씁니다. 출력은 `C:\Unity Build\ITCH_DEMO` 이고
경로와 압축은 코드가 정합니다. **유니티의 Build 버튼을 쓰지 마세요** - 이유는 Steam 절 ③ 참고.
...
`--userversion` 은 `<bundleVersion>-<커밋해시>` 로 자동 조립됩니다. itch에는 SteamPipe의
`desc` 에 해당하는 자리가 없어서, **나중에 어느 빌드가 어느 코드였는지 아는 유일한 단서**가
이 값입니다. Steam 쪽에서 `desc` 에 커밋 해시를 박는 습관과 같습니다.

버전은 스크립트에 적지 않고 **`ProjectSettings.asset` 의 `bundleVersion` 을 직접 읽습니다.**
한 번 하드코딩해 뒀다가 `1.0.0` → `1.0.1` 로 올린 뒤 1.0.1 빌드를 1.0.0 으로 올릴 뻔했습니다.
읽지 못하면 업로드하지 않고 멈춥니다 - 틀린 값으로 올리느니 안 올리는 편이 낫습니다.
...
업로드 직후에 `butler status` 를 부르면 `No channel found` 가 나오는데 정상입니다.
몇 분 뒤 다시 확인하면 `BUILD` 칸에 체크 표시와 함께 채널이 뜹니다.

```
CHANNEL   UPLOAD      BUILD        VERSION
windows   #19179180   V #1965017   1.0.0-b3357ac0
```

채널이 생긴 뒤, 파트너 사이트가 아니라 게임 페이지에서 직접 봅니다.
...
- **처리 중인데 실패로 오해** — 업로드 직후의 `No channel found` 는 정상입니다. 3번 참고

---

## 신규 계정은 첫 빌드에서 격리될 수 있습니다

itch 는 신규 계정이 악성코드를 게임으로 위장해 올리는 사기에 대응해, **계정과 페이지가 새것인
상태에서 첫 Windows 빌드가 올라오면 페이지를 자동 격리**합니다. 방문자에게 경고창이 뜹니다.

**페이지 완성도는 이 판정에 반영되지 않습니다.** 사기꾼도 실제 게임의 스크린샷과 트레일러를
훔쳐 쓰기 때문에, itch 는 위조할 수 없는 신호만 봅니다 — 계정 나이, 외부에서의 실재 증거.

풀려면 `support@itch.io` 에 **계정을 소유한 이메일 주소로** 검토를 요청합니다. 다른 주소로
보내면 처리되지 않습니다. 근거로는 **Steam 스토어 페이지**가 가장 강력합니다. 두 페이지의
Developer/Publisher 가 itch 계정명과 같다는 점을 명시하세요.

해제될 때까지 **홍보하지 마세요.** 지금 들어오는 사람은 게임보다 경고창을 먼저 봅니다.
페이지는 Published 로 두십시오 — 검토자가 봐야 합니다.
